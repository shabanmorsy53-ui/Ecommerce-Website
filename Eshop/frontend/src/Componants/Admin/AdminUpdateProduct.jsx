...
export default function AdminUpdateProduct() {
  ...
  useEffect(() => {
    if (product.data) {
      ...
      setBriceBefore(product.data.price);
      setImages({});
      setProdQty(product.data.availableQuantity);
      setSelectedCat(product.data.category);
    }
  }, [product.data]);
  ...
  useEffect(() => {
    if (selectedCatId !== "0") {
      const run = async () => {
        await dispatch(getDependSubCategory(selectedCatId));
      };
...
//   }, [loading]);
  ...
  return (
    <div>
      <div className="slider-title text-end mb-3">تعديل المنتج</div>
      ...
      <div className="text-start ">
        <button onClick={onSubmit} className="btn btn-dark">حفظ التعديلات</button>
      </div>
